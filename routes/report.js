...
router.post('/update', function (req, res, next) {
  ...
  getConn((err, db) => {
    if (err) throw err;
    let user = db.db('hip');
    let place = user.collection('report');

    place.find({ place_id: place_id }).toArray((err, result) => {
      console.log('별별별');
      console.log(result);

      if (err) throw err;
      if (result.length > 0) {
        req.num = result[0].num;
        next();
      } else {
        req.num = -1;
        next();
...
router.post('/update', function (req, res, next) {
  ...
  getConn((err, db) => {
    ...
    report.updateOne(
        ...
        (err, result) => {
          if (err) throw err;
          res.redirect(`/report/read/${place_id}/${num}`);
        }
    );
  });
});
